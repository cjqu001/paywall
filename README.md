...
A self-hosted server that bypasses paywalls and blocks ads on sites like Medium, New York Times, and others.

## What is this?

This is a simple self-hosted server with a clean interface to bypass paywalls, block ads, and get around other access restrictions. If you just want to read one article and move on with your day, this might be helpful.
...
```sh
git clone https://github.com/cjqu001/paywall.git
cd paywall
docker compose up
```
...
```javascript
javascript:(function(){window.location.href='https://your-instance-url/'+encodeURIComponent(window.location.href);})();
```
Replace `https://your-instance-url` with the address of your own hosted instance.
...
Enter Bookmark Details:
    - Name: Enter a name for your bookmarklet, such as "Paywall Bypass". This name will show as a bookmark title in the bookmarks bar
    - URL: Paste the JavaScript code provided above into the URL field.
4. Click Save.

Using the Bookmarklet:

Navigate to the page whose URL you want to bypass.

Click on the bookmarklet you saved in your bookmarks bar. The browser will redirect you to the bypassed version of the URL using your service.

To show Bookmarks in Chrome, click the icon with three horizontal bars in the top right corner to open options. In options, hover over "Bookmarks" to display a second menu where you can click the "Show bookmarks bar" text to toggle the bar on or off.
...
```
/lib/systemd/system/paywall.service
```

```
[Unit]
Description=Paywall Bypass Flask Service
...
```
<VirtualHost *:22114>
    ErrorLog ${APACHE_LOG_DIR}/paywall-error.log
    CustomLog ${APACHE_LOG_DIR}/paywall-access.log combined
